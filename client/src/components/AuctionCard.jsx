...
export default function AuctionCard({ id, title, staringPrice, endsAt, highestBid, description, imageURL, hostId, profileId, handleDelete, handleOpenUpdateModal, onClick }) {
    ...
    const timeRemaining = formatDistanceToNow(new Date(endsAt));
            ...
            <div className="p-5">
                ...
                <div className="mt-5 flex flex-wrap items-center justify-end gap-3">
                    {(isHost && !hasEnded) ? (
                        ...
                        <button onClick={onClick} disabled={hasEnded} className="flex items-center gap-2 rounded-xl bg-[#22D3EE] px-3 py-2 text-sm font-medium text-[#111827] transition hover:opacity-90">
                            <FaGavel /> Bid Now
                        </button>
                    )}
                </div>
